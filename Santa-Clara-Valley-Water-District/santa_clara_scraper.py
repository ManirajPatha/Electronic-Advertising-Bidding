...
URL = "https://vendors.planetbids.com/portal/48397/bo/bo-search"
OUTPUT_JSON = "santa_clara_valley_bids_bidding.json"
WAIT_TIME = 30
...
def select_stage_bidding(driver: webdriver.Chrome, wait: WebDriverWait) -> None:
    """
    1) Wait for the Stage select element: <select id="stageId-field" ...>
    2) Choose Bidding (value = "3")
    3) Click the Search button: <button class="search-btn" type="submit">Search</button>
    """
...
def get_results_table(driver: webdriver.Chrome, wait: WebDriverWait):
    """
    Return the results table element.
    We just look for ANY table that has at least one <tbody><tr> row.
    """
...
def scrape_bid_information(driver: webdriver.Chrome, wait: WebDriverWait) -> Dict:
    """Scrape the Bid Information tab using the HTML you provided."""
...
def scrape_prospective_bidders(
    ...
    """
    Scrape the 'Prospective Bidders' tab and return a list of vendor dicts.

    Vendor column is stored as a list of lines (no '\\n' in JSON).
    """
...
def scrape_one_bid(driver: webdriver.Chrome, wait: WebDriverWait) -> Dict:
    """
    Called when already on a bid-detail page.
    Scrapes Bid Information & Prospective Bidders and returns a dict for this bid.
    """
...
def go_back_to_search(driver: webdriver.Chrome, wait: WebDriverWait) -> None:
    """
    Click the "Back to Bid Search" link then wait again for the results table.
    """
    ...
    back_link = wait.until(
        EC.element_to_be_clickable(
            (
                By.XPATH,
                "//a[contains(@href,'/portal/48397/bo/bo-search') "
...
def scrape_all_bids(driver: webdriver.Chrome, wait: WebDriverWait) -> List[Dict]:
    """Scrape all rows in the Stage=Bidding result table."""
