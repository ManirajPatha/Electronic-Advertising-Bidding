...
class HumboldtCountyScraper:
    def __init__(self):
        ...
        self.base_url = "https://humboldtgov.org/Bids.aspx"
        self.base_domain = "https://humboldtgov.org"
    ...
    def extract_bid_details(self, detail_url):
        ...
        try:
            ...
            print(f"Error extracting bid details from {detail_url}: {e}")
            import traceback
            traceback.print_exc()
            return {}
                    ...
                    if title_div:
                        ...
                        if title_link:
                            ...
                            if bid_info['detail_link'] and not bid_info['detail_link'].startswith('http'):
                                bid_info['detail_link'] = f"{self.base_domain}/{bid_info['detail_link'].lstrip('/')}"
    ...
    def scrape_all_bids(self):
        """Main method to scrape all bids from the Humboldt County portal"""
        try:
            ...
            print(f"Found {len(category_divs)} categories")
            ...
            print(f"Found {len(all_bids)} total bids")
          
            detailed_bids = []
            for idx, bid in enumerate(all_bids, 1):
                try:
                    print(f"Processing bid {idx}/{len(all_bids)}: {bid.get('bid_number', 'N/A')}")
                    ...
                    print(f"Error processing bid {idx}: {e}")
                    import traceback
                    traceback.print_exc()
                    detailed_bids.append(bid)
                    continue
            ...
            print(f"Error in scrape_all_bids: {e}")
            import traceback
            traceback.print_exc()
            return []
  
    def save_to_json(self, data, filename='humboldt_county_bids.json'):
...
def main():
    ...
    scraper = HumboldtCountyScraper()
  
    try:
        print("Starting to scrape Humboldt County bids...")
        bids = scraper.scrape_all_bids()
        print(f"\nScraped {len(bids)} bids successfully")
      
        scraper.save_to_json(bids)
        print(f"Data saved to humboldt_county_bids.json")
